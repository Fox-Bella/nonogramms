...
class Square(Square):
    ...
    def drawIJ(self, scene, i, j, i_cells, j_cells):
        ...
        if self.enabled:
            ...
            pygame.draw.rect(scene, self.color_line, (self.x + inline, self.y + inline, self.size - inline * 2, self.size - inline * 2))

        if i % i_cells == 0:
            pygame.draw.line(scene, self.color_line_outline, (self.x - 1, self.y), (self.x - 1, self.y + self.size), self.width_line)
        elif (i + 1) % i_cells == 0:
            pygame.draw.line(scene, self.color_line_outline, (self.x  - 1 + self.size, self.y), (self.x + self.size - 1, self.y + self.size), self.width_line)

        if j % j_cells == 0:
            pygame.draw.line(scene, self.color_line_outline, (self.x, self.y - 1), (self.x + self.size, self.y - 1), self.width_line)
        elif (j + 1) % j_cells == 0:
