...
class Square:
    ...
    def drawIJ(self, scene: pygame, i, j):
        ...
        if ((i - setup.BLANK_FIELD) / self.size) % 5 == 0:
            pygame.draw.line(scene, Square.color_line, (i, j), (i, j + self.size), 2)
        elif ((i + self.size - setup.BLANK_FIELD) / self.size) % 5 == 0:
            pygame.draw.line(scene, Square.color_line, (i + self.size, j), (i + self.size, j + self.size), 2)

        if ((j - setup.BLANK_FIELD) / self.size) % 5 == 0:
            pygame.draw.line(scene, Square.color_line, (i, j), (i + self.size, j), 2)
        elif ((j + self.size - setup.BLANK_FIELD) / self.size) % 5 == 0:
            pygame.draw.line(scene, Square.color_line, (i, j + self.size), (i + self.size, j + self.size), 2)
